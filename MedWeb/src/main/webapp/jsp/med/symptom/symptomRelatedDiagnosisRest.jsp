			...
			function getDiagnosis() {
				$('#searchsympdiag').click(function() {
					...
					if (!blankSearchCriteria) {
						$.ajax({
						    type: 'GET',
						    url: "http://localhost:8182/graphs/patientgraph/tp/gremlin?script=pagerank(" + symptomduration + ", '" + age + "')&load=[subgraph]",
						    datatype: 'application/json',
						    cache: 'false',
						    success: function(response) {
						       var results = response.results;
						       var html = "<div><div style='background-color:#b0c4de;'>";
						       html = html + "<div><b># of diagnosis : </b>" + results.length + "</div>";
						       html = html + "<div><b>Query Time : </b>" + response.queryTime + " ms </div>";
						       html = html + "<div><b>Top 20 Diagnosis</b></div>";
						       html = html + "</div><div><table>";
						       $.each(results, function(i, item) {
						    	   html = html + "<tr><td><u>"+results[i][0]+"</u></td></tr>";
						    	   return i < 20;
						       });
